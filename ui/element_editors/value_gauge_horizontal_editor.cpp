#include "value_gauge_horizontal_editor.h"
#include "ui_value_gauge_horizontal_editor.h"

QTBValueGaugeHorizontalEditor::QTBValueGaugeHorizontalEditor(QTBValueGaugeHorizontal *display, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::QTBValueGaugeHorizontalEditor),
    mGauge(display)
{
    ui->setupUi(this);
    ...
    connect(ui->buttonBox->button(QDialogButtonBox::Apply), &QPushButton::clicked,
            this, &QTBValueGaugeHorizontalEditor::updateElement);
    ...
    connect(mPropertiesWidget, &PropertiesWidget::connectProperties,
            this, &QTBValueGaugeHorizontalEditor::connectProperties);
...
QTBValueGaugeHorizontalEditor::~QTBValueGaugeHorizontalEditor()
{
    delete ui;
}

void QTBValueGaugeHorizontalEditor::accept()
{
    updateElement();
    QDialog::accept();
}

void QTBValueGaugeHorizontalEditor::updateElement()
{
    if(mGauge) {
        ...
        mGauge->setAxisScale(QTBValueGaugeHorizontal::AxisScale(ui->rangeComboBox->currentIndex()));
...
void QTBValueGaugeHorizontalEditor::connectProperties(bool connected)
{
    QSharedPointer<QTBDashboardParameter> dashParam = mGauge->dashParameter(0);
    if(dashParam) {
        if(connected) {
...
void QTBValueGaugeHorizontalEditor::newParameter()
{
...
void QTBValueGaugeHorizontalEditor::removeParameter()
{
    mGauge->removeDashParameter(0);
    ui->tabWidget->removeTab(1);
}

void QTBValueGaugeHorizontalEditor::on_rangeComboBox_currentIndexChanged(int index)
{
    if(index == 2)
...
void QTBValueGaugeHorizontalEditor::on_listWidget_currentRowChanged(int currentRow)
{
