...
*/
  ...
  \see setMarginValue
*/
    inline int getMarginValue(const QMarginsF &margins, QCP::MarginSide side)
    {
        switch (side)
        {
...
protected:
    ...
    QWeakPointer<QOpenGLFunctions> mGlFunctions;
...
protected:
    ...
    // non-virtual methods:
    QLineF getRectClippedStraightLine(const QCPVector2D &point1, const QCPVector2D &vec, const QRectF &rect) const;
    QPen mainPen() const;
};
...
protected:
    ...
    // non-virtual methods:
    void updateScaledPixmap(QRectF finalRect=QRect(), bool flipHorz=false, bool flipVert=false);
