...
void PlotTimeGraph::addColoredSegment(double lowerValue, double upperValue, QPen pen, QBrush brush)
{
    if(qFuzzyCompare(lowerValue, upperValue)) {
        logger()->debug() << "Invalid segment : identical values";
        return;
    }
...
{
    if (!mKeyAxis || !mValueAxis) { logger()->debug() << Q_FUNC_INFO << "invalid key or value axis"; return; }
    ...
    for(int index = 0; index < mLowerValues.count(); index++) {
        ...
        for (QCPGraphDataContainer::const_iterator it=mDataContainer->constBegin(); it!=mDataContainer->constEnd(); ++it) {
            if(it->value >= mLowerValues.at(index) && it->value < mUpperValues.at(index)) {
                ...
                if(begin != mDataContainer->constEnd()) {
                    ...
                    allSegments.append(GraphColoredSegment(QCPDataRange(begin-mDataContainer->constBegin(), it-mDataContainer->constBegin()), mPens.at(index), mBrushs.at(index)));
        ...
        if(begin != mDataContainer->constEnd()) {
            ...
            allSegments.append(GraphColoredSegment(QCPDataRange(begin-mDataContainer->constBegin(), dataCount()), mPens.at(index), mBrushs.at(index)));
        }
    }
...
QPointF PlotTimeGraph::getCustomFillBasePoint(QPointF matchingDataPoint) const
{
    ...
    if (!keyAxis || !valueAxis) { logger()->debug() << Q_FUNC_INFO << "invalid key or value axis"; return QPointF(); }
