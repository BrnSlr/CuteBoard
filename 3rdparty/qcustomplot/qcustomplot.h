...
double QCPAbstractPlottable1D<DataType>::dataMainKey(int index) const
{
    ...
    {
        logger()->debug() << Q_FUNC_INFO << "Index out of bounds" << index;
        return 0;
    }
}
...
public:
  ...
  */
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details=nullptr) const; // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for plottables
    virtual QCPPlottableInterface1D *interface1D() { return nullptr; } // TODO: return this later, when QCPAbstractPolarPlottable is created
